...
async def _historical_registration_asset_mutation_disabled():
    ...
    logger.info("Fetching GLPI inventory from database to find missing AM codes...")
    try:
        ...
        glpi_assets = {str(r[1]).strip().lower() for r in rows}
        conn.close()
    ...
    for idx, row in df.iterrows():
        ...
        if asset_id and asset_id.lower() not in glpi_assets:
            missing_to_create.append((idx, asset_id))

    logger.info(f"Found {len(missing_to_create)} missing AM- codes. Creating them now...")
    ...
    logger.info("Successfully created and synced all missing AM- codes!")
